...
import { MAX_PLACEMENT_DEVICE_COUNT } from "../planning/placement-limits.js";
...
function positiveIntegerOrDefault(value, label, max, fallback) {
  if (value === null || value === undefined) return fallback;
  const number = Number(value);
  if (!Number.isFinite(number) || number <= 0 || !Number.isInteger(number) || number > max) {
    return fallback;
  }
  return number;
}
...
export function validateDeviceRequest(value) {
  ...
  const count = positiveIntegerOrDefault(input.count, "count", MAX_PLACEMENT_DEVICE_COUNT, 1);
