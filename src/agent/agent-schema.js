...
function nonNegativeNumber(value, label, { integer = false, max = Number.POSITIVE_INFINITY } = {}) {
  if (value === null || value === undefined) {
    throw invalid(`${label} is required`, { field: label });
  }
  const number = Number(value);
  if (!Number.isFinite(number) || number < 0 || number > max || (integer && !Number.isInteger(number))) {
    throw invalid(`${label} is invalid`, { field: label });
  }
  return number;
}

function positiveIntegerOrDefault(value, label, fallback) {
  if (value === null || value === undefined) return fallback;
  if (typeof value !== "number" || !Number.isInteger(value) || value <= 0) {
    throw invalid(`${label} must be a positive integer`, { field: label });
  }
  return value;
}
...
export function validateDeviceRequest(value) {
  ...
  const count = positiveIntegerOrDefault(input.count, "count", 1);
  // u_size 是关键字段，必须由用户明确提供；缺失则无法规划
  if (input.u_size === null || input.u_size === undefined) {
    throw invalid("u_size is required", { field: "u_size" });
  }
  return {
    id,
    it_code: nullableString(input.it_code),
    count,
    u_size: positiveNumber(input.u_size, "u_size", { integer: true, max: 10 }),
    rated_power_w: nonNegativeNumber(input.rated_power_w, "rated_power_w"),
    real_power_w: null,
    weight_kg: nonNegativeNumber(input.weight_kg, "weight_kg"),
    network_ports: nonNegativeNumber(input.network_ports, "network_ports", { integer: true, max: 256 }),
    ...
    business: nullableString(input.business),
...
function validatePlacementDevice(value, startU) {
  ...
  const allowed = [
    "id", "it_code", "u_size", "start_u", "rated_power_w", "real_power_w", "weight_kg", "network_ports",
  ...
  for (const field of ["it_code", "real_power_w", "business_id", "replica_group", "hostname", "model", "business", "owner"]) {
