...
export function createDiagnosisEngine({
  ...
  function diagnose(alarmId, actor) {
    ...
    const diagnosis = {
      ...
      recommended_actions: remediationActions(alarm, state),
      status: "completed",
      created_at: now(),
      created_by: actor,
      plan_id: null,
  ...
  function remediationActions(alarm, state) {
    if (alarm.scenario === "power_high") {
      const device = state.devices.find((item) => item.id === "DEMO-ALARM-POWER");
      if (!device) throw diagnosisError("DEVICE_NOT_FOUND", "Alarm remediation device does not exist", { device_id: "DEMO-ALARM-POWER" });
      // CAB-19 is a 20kW rack with enough headroom for the 8.2kW alarm demo device
      // after CAB-11~CAB-13 were redistributed to 10kW.
      const targetRackId = "CAB-19";
      const links = allocateFirstCompatibleConnections(state, device, targetRackId, {
        existingNetworkConnections: state.network_connections.filter((item) => item.device_id === device.id),
      });
      return [{
        type: "move_device",
        device_id: device.id,
        from: { rack_id: device.rack_id, layer_id: device.layer_id, start_u: device.start_u },
        to: { rack_id: targetRackId, layer_id: "L04", start_u: 33 },
  ...
  async function createRemediation(alarmId, actor) {
    ...
    const plan = await planService.create({
      ...
      actions: remediationActions(alarm, state),
  ...
  async function confirmRemediation(planId, actor) {
    ...
    let result;
    try {
      result = await planService.confirm(planId, actor);
    } catch (error) {
      const current = repository.read().alarms.find((item) => item.id === alarmId);
      if (current?.status === "executing") {
        alarmEngine.markActionPendingAfterFailure(alarmId, actor);
      }
      throw error;
    }
