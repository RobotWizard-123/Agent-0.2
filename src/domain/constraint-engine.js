...
import { validateLinkResources } from "./link-resources.js";
import { planLayerCompaction } from "./layer-compaction.js";
...
function assertSourceSnapshot(device, from) {
  if (!from
    || device.rack_id !== from.rack_id
    || device.layer_id !== from.layer_id
    || Number(device.start_u) !== Number(from.start_u)) {
    throw actionError("PLAN_STALE", `${device.id} no longer matches the planned source position`, {
      device_id: device.id,
      expected: structuredClone(from ?? null),
      actual: {
        rack_id: device.rack_id,
        layer_id: device.layer_id,
        start_u: Number(device.start_u),
      },
    });
  }
}

function replaceDeviceConnections(state, device, powerConnections = [], networkConnections = []) {
  state.power_connections = state.power_connections.filter((item) => item.device_id !== device.id);
  state.network_connections = state.network_connections.filter((item) => item.device_id !== device.id);
  state.power_connections.push(...powerConnections.map((item) => ({
    ...structuredClone(item),
    device_id: device.id,
    source_id: item.source_id ?? state.racks.find((rack) => rack.id === device.rack_id)?.source_id ?? null,
    rack_id: device.rack_id,
    data_source: item.data_source ?? "planned",
  })));
  state.network_connections.push(...networkConnections.map((item) => ({
    ...structuredClone(item),
    device_id: device.id,
    data_source: item.data_source ?? "planned",
  })));
...
function exactCompactionMoves(expected, supplied) {
  return Array.isArray(supplied)
    && JSON.stringify(expected) === JSON.stringify(supplied);
}

function compactionMismatch(expected, supplied, removed) {
  const submitted = Array.isArray(supplied) ? supplied : [];
  const mismatch = expected.find((item, index) =>
    JSON.stringify(item) !== JSON.stringify(submitted[index]))
    ?? submitted.find((item, index) => JSON.stringify(item) !== JSON.stringify(expected[index]));
  throw actionError("COMPACTION_FAILED", `Compaction plan is stale or invalid for ${removed.id}`, {
    device_id: mismatch?.device_id ?? removed.id,
...
export function applyActions(state, actions) {
  ...
  for (const action of actions) {
    ...
    if (action.type === "move_device") {
      ...
      const rack = findRack(projected, action.to?.rack_id);
    ...
    if (action.type === "remove_device") {
      ...
      const expectedMoves = planLayerCompaction(projected, device.id);
      if (Object.hasOwn(action, "compaction_moves")
        && !exactCompactionMoves(expectedMoves, action.compaction_moves)) {
        compactionMismatch(expectedMoves, action.compaction_moves, device);
      }
      ...
      for (const move of expectedMoves) {
        const peer = projected.devices.find((item) => item.id === move.device_id);
        if (!peer
          || peer.rack_id !== move.rack_id
          || peer.layer_id !== move.layer_id
          || Number(peer.start_u) !== Number(move.from_start_u)) {
          throw actionError("COMPACTION_FAILED", `Compaction source changed for ${move.device_id}`, {
            device_id: move.device_id,
            rack_id: move.rack_id,
            layer_id: move.layer_id,
          });
        }
        peer.start_u = Number(move.to_start_u);
      }
      const rack = findRack(projected, device.rack_id);
      try {
        packDevices(rack, projected.devices.filter((item) =>
          item.status !== "cancelled"
          && item.rack_id === device.rack_id
          && item.layer_id === device.layer_id));
      } catch (error) {
        throw actionError("COMPACTION_FAILED", `Compacted layer is invalid for ${device.id}`, {
          device_id: error.device_id ?? device.id,
          rack_id: device.rack_id,
          layer_id: device.layer_id,
          cause_code: error.code ?? "LAYOUT_INVALID",
        });
      }
      continue;
    }
...
function affectedRackIds(state, actions) {
  ...
  for (const action of actions) {
    ...
    if (action.to?.rack_id) ids.add(action.to.rack_id);
...
function validateRack(state, rack, blockers, warnings) {
  ...
  for (const device of devices) {
    for (const [field, options] of [
      ["rated_power_w", { integer: false }],
      ["weight_kg", { integer: false }],
      ["network_ports", { integer: true }],
    ]) {
      const value = Number(device[field]);
      if (!Number.isFinite(value) || value < 0 || (options.integer && !Number.isInteger(value))) {
        blockers.push(issue("DEVICE_CAPACITY_FACT_INVALID", device.id, `${device.id} has an invalid ${field}`, "blocker", {
          device_id: device.id,
          field,
...
const PLANNING_FIELDS = ["racks", "devices", "power_connections", "network_connections", "topology", "demo_conditions", "service_health"];

export function slimForPlanning(state) {
  const slim = {};
  for (const field of PLANNING_FIELDS) {
    if (Object.hasOwn(state, field)) slim[field] = structuredClone(state[field]);
  }
  if (Object.hasOwn(state, "version")) slim.version = state.version;
  return slim;
}
...
export function evaluateActions(state, actions = []) {
  ...
  for (const rackId of rackIds) {
    const rack = projectedState.racks.find((item) => item.id === rackId);
    if (rack) validateRack(projectedState, rack, blockers, warnings);
  }
  const affectedDeviceIds = new Set(actions
    .filter((action) => ["place_device", "move_device"].includes(action.type))
    .map((action) => action.device_id ?? action.device?.id)
    .filter(Boolean));
  blockers.push(...validateLinkResources(projectedState, affectedDeviceIds));
...
export function evaluateState(state) {
  const blockers = [];
  const warnings = [];
  const rackIds = state.racks.map((rack) => rack.id);
  for (const rack of state.racks) validateRack(state, rack, blockers, warnings);
  blockers.push(...validateLinkResources(state));
  validateReplicaDomains(state, blockers);
  const current = snapshots(state, rackIds);
  return {
    allowed: blockers.length === 0,
    blockers,
    warnings,
