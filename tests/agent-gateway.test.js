...
test("natural-language failure requests the structured form without fabricating a plan", async () => {
  ...
  assert.equal(result.agent_status, "degraded");
  assert.equal(result.error_code, "AGENT_UNAVAILABLE");
  assert.deepEqual(result.missing_fields, ["u_size", "rated_power_w", "weight_kg", "network_ports"]);
