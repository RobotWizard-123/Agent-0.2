...
test("viewer cannot change the selected candidate of an existing plan", async () => {
  await withAuthenticatedServer(async ({ login, request }) => {
    ...
    const candidate = created.body.original_candidates[1];
    const selected = await request(`/api/plans/${created.body.id}/select-candidate`, {
      method: "POST",
    ...
    assert.equal(selected.body.error, "FORBIDDEN");
  });
});
