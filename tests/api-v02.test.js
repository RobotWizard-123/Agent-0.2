...
test("V0.2 API reports an occupied manual target without creating a plan", async () => {
...
test("V0.2 API rejects incomplete and multi-device manual requests", async () => {
  await withV02Server(async ({ repository, request }) => {
    for (const [body, code] of [
      [{ ...placementRequest(), placement_mode: "manual" }, "MANUAL_TARGET_REQUIRED"],
      [{ ...placementRequest(), placement_mode: "manual-experimental" }, "PLACEMENT_MODE_INVALID"],
      [{
        ...placementRequest(),
        placement_mode: "manual",
        target_position: { rack_id: "CAB-03", layer_id: "L03", start_u: 23 },
        device: { ...placementRequest().device, count: 2 },
      }, "MANUAL_PLACEMENT_COUNT_INVALID"],
    ]) {
      const result = await request("/api/plans", { method: "POST", body });
...
test("planning terminal errors use bounded HTTP 422 details", async () => {
  const planner = {
    recommend(_state, device) {
      if (device.id === "BUDGET-ERROR") {
        throw Object.assign(new Error("model too large"), {
          code: "MODEL_BUDGET_EXCEEDED",
          request_count: 500,
          assignment_variables: 20_001,
          budget: 20_000,
          solver_input: { must_not_leak: true },
        });
      }
      throw Object.assign(new Error("no plan"), {
        code: "NO_RECONFIGURATION_PLAN",
        request_count: 3,
  ...
  await withV02Server(async ({ request }) => {
    const budget = await request("/api/plans", {
      method: "POST",
      body: {
        ...placementRequest(),
        device: { ...placementRequest().device, id: "BUDGET-ERROR" },
      },
    });
    assert.equal(budget.response.status, 422);
    assert.equal(budget.body.error, "MODEL_BUDGET_EXCEEDED");
    assert.deepEqual(budget.body.details, {
      request_count: 500,
      assignment_variables: 20_001,
      budget: 20_000,
    });

    const noPlan = await request("/api/plans", {
      method: "POST",
      body: {
        ...placementRequest(),
        device: { ...placementRequest().device, id: "NO-PLAN-ERROR" },
      },
    });
    assert.equal(noPlan.response.status, 422);
    assert.equal(noPlan.body.error, "NO_RECONFIGURATION_PLAN");
    assert.deepEqual(noPlan.body.details, { request_count: 3 });
  }, { planner });
});
