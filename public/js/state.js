...
const state = {
  ...
  agentStatus: null,           // { status, model, base_url, error_code, ... } from /api/agent/status
  selectedRack: null,
...
export async function refreshRoom() {
  ...
  try {
    const [room, racks, alarms, topologyPower, topologyNetwork, config, placementSettings, agentStatus] = await Promise.all([
      get("/api/room"),
      get("/api/racks"),
      get("/api/alarms"),
      ...
      get("/api/agent/status").catch(() => null),
    ]);
    const selectedAlarm = state.selectedAlarm
      ? alarms.items.find((alarm) => alarm.id === state.selectedAlarm.id) ?? state.selectedAlarm
      : null;
    assign({ room, racks: racks.items, alarms: alarms.items, topologyPower, topologyNetwork, config, placementSettings, agentStatus, selectedAlarm, loading: false });
  } catch (error) {
...
export async function refreshAgentStatus({ force = false } = {}) {
  try {
    const agentStatus = await get(`/api/agent/status${force ? "?force=1" : ""}`);
    assign({ agentStatus });
    return agentStatus;
  } catch (error) {
    // Keep the last known status; do not blow up the page.
...
export async function selectPlanCandidate(planId, candidateId) {
  const updated = await post(`/api/plans/${encodeURIComponent(planId)}/select-candidate`, { candidate_id: candidateId });
  assign({ currentPlan: updated, lastExecution: null });
  return updated;
}
