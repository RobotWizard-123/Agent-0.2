...
export async function loadTopology(query = "") {
  ...
  if (!normalized) {
    ...
    const topologyResult = { kind: "aggregate", query: normalized, power, network };
    if (requestSerial === topologyRequestSerial) assign({ topologyResult });
    return topologyResult;
  }

  if (normalized) {
    const exactMatches = state.devices.filter((item) =>
      item.id.toUpperCase() === upper ||
      (item.it_code && item.it_code.toUpperCase() === upper) ||
      (item.hostname && item.hostname.toUpperCase() === upper),
    );
    const partialMatches = state.devices.filter((item) =>
      item.id.toUpperCase().includes(upper) ||
      (item.it_code && item.it_code.toUpperCase().includes(upper)) ||
      (item.hostname && item.hostname.toUpperCase().includes(upper)),
    );
    ...
    if (exactMatches.length === 1) {
      const topologyResult = {
        kind: "device",
        query: normalized,
        device: await get(`/api/topology/devices/${encodeURIComponent(exactMatches[0].id)}`),
      };
      if (requestSerial === topologyRequestSerial) assign({ topologyResult });
      return topologyResult;
    }

    const devicesToList = exactMatches.length > 1 ? exactMatches : partialMatches;
    if (devicesToList.length > 0) {
      const selectedId = devicesToList[0].id;
      const selectedPath = await get(`/api/topology/devices/${encodeURIComponent(selectedId)}`);
      const topologyResult = {
        kind: "multi-device",
        query: normalized,
        devices: devicesToList.map((device) => ({ ...device })),
        selectedId,
        selectedPath,
      };
      if (requestSerial === topologyRequestSerial) assign({ topologyResult });
    ...
    const rackMatches = state.racks.filter((item) =>
      item.id.toUpperCase() === upper ||
      (item.name && item.name.toUpperCase() === upper) ||
      (item.alias && item.alias.toUpperCase() === upper),
    );
    if (rackMatches.length === 1) {
      const rack = await get(`/api/racks/${encodeURIComponent(rackMatches[0].id)}`);
      const topologyResult = { kind: "rack", query: normalized, rack };
      if (requestSerial === topologyRequestSerial) assign({ topologyResult });
      return topologyResult;
    }
  }
  ...
  const device = state.devices.find((item) => item.id.toUpperCase() === upper || (item.it_code && item.it_code.toUpperCase() === upper) || (item.hostname && item.hostname.toUpperCase() === upper));
  if (device) {
    resolved = device.id;
  } else {
