...
export function createAssistantClient({
  ...
  async function poll() {
    ...
    try {
      ...
      // Also pull real agent connectivity on every feedback poll so the
      // assistant panel's status reflects the same probe as the top bar.
      void syncAgentStatus({ force: false });
      return result;
    } catch (error) {
  ...
  async function syncAgentStatus({ force = false } = {}) {
    try {
      const result = await getImpl(`/api/agent/status${force ? "?force=1" : ""}`);
      const next = result?.status === "unconfigured"
        ? "not_configured"
        : result?.status === "available"
          ...
          : result?.status === "degraded"
            ? "degraded"
            : state.model_status;
      if (next && next !== state.model_status) {
        patch({ model_status: next });
      }
    } catch {
      /* keep last known status; do not surface this as a chat error */
    }
  }
  ...
  return {
    ...
    async setSession(session) {
      ...
      try {
        ...
        // Pull the real agent status right after auth so the panel's badge
        // is truthful from the first frame.
        void syncAgentStatus({ force: true });
      } catch (error) {
    ...
    poll,
    async send(message) {
      if (!state.authenticated) throw clientError("AUTH_REQUIRED", "Login is required");
      if (typeof message !== "string" || !message.trim()) {
        throw clientError("ASSISTANT_INPUT_INVALID", "Message is required");
      }
    ...
    async acceptProposal(proposalId) {
      if (state.role !== "admin") {
        throw clientError("FORBIDDEN", "Administrator role is required");
      }
