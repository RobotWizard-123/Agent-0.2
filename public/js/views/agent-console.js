...
function structuredForm(state) {
  ...
  const form = h("form", { className: "agent-form" }, [
    ...
    h("div", { className: "form-grid" }, [
      ...
      h("label", { className: "form-field" }, ["数量", h("input", { name: "count", type: "number", min: 1, max: 10, value: 1, required: true })]),
...
export function renderAgentConsole(container, state) {
  ...
  const intro = h("section", { className: "view-intro" }, [
    h("div", {}, [
      h("p", { className: "section-kicker", text: "PLAN → VALIDATE → AGENT REVIEW → CONFIRM → EXECUTE → VERIFY" }),
  ...
  const result = panel("候选方案与风险", renderPlacementPlan(state.currentPlan, state), { kicker: "RULE + AGENT REVIEW" });
