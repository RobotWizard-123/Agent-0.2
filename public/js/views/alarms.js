...
import { alarmTriggerLabel, alarmObjectTypeLabel } from "../alarm-labels.js";
...
function alarmList(state) {
  ...
  return h("div", { className: "alarm-list" }, state.alarms.map((alarm) => h("button", {
    ...
    h("span", {}, [h("strong", { text: alarmTriggerLabel(alarm.trigger_code) }), h("small", { text: `${alarm.object_id} · ${new Date(alarm.opened_at).toLocaleString("zh-CN")}` })]),
...
function alarmDetail(state) {
  ...
  return h("div", { className: "alarm-detail" }, [
    h("div", { className: "alarm-detail-head" }, [
      h("div", {}, [h("p", { className: "section-kicker", text: alarm.id }), h("h2", { text: alarmTriggerLabel(alarm.trigger_code) }), h("p", { text: `${alarmObjectTypeLabel(alarm.object_type)} / ${alarm.object_id}` })]),
...
export function renderAlarms(container, state) {
  const intro = h("section", { className: "view-intro" }, [
    h("div", {}, [h("p", { className: "section-kicker", text: "触发 → 诊断 → 处置 → 验证" }), h("h2", { text: "报警诊断与恢复" }), h("p", { text: "报警不能被直接标记为恢复。只有处置执行后触发条件确实消失，系统才会关闭报警。" })]),
    statusBadge(state.session.role === "admin" ? "管理员演示模式" : "只读模式", state.session.role === "admin" ? "ok" : "neutral"),
  ]);
  const scenariosPanel = panel("可交互演示场景", scenarioButtons(state), { kicker: "演示触发" });
  const listPanel = panel("报警队列", alarmList(state), { kicker: "报警" });
  const detailPanel = panel("诊断工作区", alarmDetail(state), { kicker: "证据与恢复" });
