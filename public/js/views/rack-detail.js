...
function deviceBlock(device, layer, options = {}) {
  const { onClick = (clicked) => loadDeviceTopology(clicked.id) } = options;
  ...
  return h("button", {
    ...
    "data-removable": device.criticality !== "critical",
...
function rackLayer(layer, options = {}) {
  ...
  return h("section", {
    ...
    h("div", { className: "device-grid" }, [
      ...
      ...layer.devices.map((device) => deviceBlock(device, layer, { onClick: options.onDeviceClick })),
      h("div", {
        className: "u-reserve",
...
export function createRackCapacityPanel(rack) {
  const capacity = rack.capacity;
  return panel("容量边界", h("div", { className: "capacity-stack" }, [
...
export function createRackVisualPanel(rack, options = {}) {
  const { onDeviceClick = (device) => loadDeviceTopology(device.id) } = options;
  return panel("42U 四层视图", h("div", { className: "rack-frame" }, [...rack.layers].reverse().map((layer) => rackLayer(layer, { onDeviceClick }))), {
    kicker: "物理机柜",
...
function deviceDrawer(topology, state) {
  ...
  const eligible = device.criticality !== "critical";
  ...
  return h("aside", { className: "device-drawer" }, [
    ...
    h("p", {
      ...
      text: eligible
        ? "取出方案会复核关键设备、业务副本与当前快照；未配置维护窗口时提示风险，并保留一次最终确认。"
        : "当前设备属于关键设备；可生成方案查看具体阻断，但不能执行。",
    }),
...
export function renderRackDetail(container, state) {
  ...
  container.replaceChildren(header, h("section", { className: "rack-detail-grid" }, [
    h("div", { className: "rack-left-column" }, [createRackCapacityPanel(rack), createRackVisualPanel(rack)]),
