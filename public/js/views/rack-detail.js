...
function deviceBlock(device, layer) {
  ...
  return h("button", {
    ...
    h("span", { className: "device-name", text: device.hostname || device.id }),
...
function deviceDrawer(topology, state) {
  if (!topology) {
    return h("aside", { className: "device-drawer is-empty" }, [
      h("p", { className: "section-kicker", text: "SERVER DETAIL" }),
      h("h3", { text: "选择一台服务器" }),
  ...
  return h("aside", { className: "device-drawer" }, [
    ...
    detailList([
      ["资产编号", device.asset_id], ["设备型号", device.model], ["序列号", device.serial],
...
export function renderRackDetail(container, state) {
  ...
  const header = h("section", { className: "rack-detail-head" }, [
    ...
    h("div", { className: "rack-title-row" }, [
      h("div", {}, [
        h("p", { className: "section-kicker", text: `${rack.source_id} / ${rack.role.toUpperCase()} RACK` }),
