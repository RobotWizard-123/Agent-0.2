...
import { alarmTriggerLabel, severityLabel } from "../alarm-labels.js";

function sum(items, field) {
  return items.reduce((total, item) => total + Number(item[field] || 0), 0);
}

function rackCard(rack, alarms, devices, tooltip) {
  const risk = rackRisk(rack, alarms);
  const usedU = rack.capacity.used_u;
  const usableU = rack.capacity.usable_u;
  const rackDevices = devices.filter((device) => device.rack_id === rack.id);

  function showTooltip(event) {
    tooltip.replaceChildren();
    if (rackDevices.length === 0) {
      tooltip.append(h("span", { className: "rack-tooltip-empty", text: "无服务器" }));
    } else {
      for (const device of rackDevices) {
        tooltip.append(h("div", {
          className: "rack-tooltip-line",
          style: { color: device.color || "var(--steel-300)" },
          text: `${device.id} / ${device.it_code || "—"}`,
        }));
      }
    }
...
function sourceGroup(sourceId, racks, alarms, devices, tooltip) {
  ...
  return h("section", { className: `source-group source-${sourceId.toLowerCase()}` }, [
    ...
    h("div", { className: "rack-row" }, items.map((rack) => rackCard(rack, alarms, devices, tooltip))),
  ]);
}
...
export function renderOverview(container, state) {
  const tooltip = h("div", { className: "rack-card-tooltip" });
  ...
  const hero = h("section", { className: "room-hero" }, [
    h("div", { className: "room-identity" }, [
      h("div", {}, [
        h("p", { className: "section-kicker", text: "L5 / A2 / 08 号机房" }),
  ...
  const floorPlan = panel("机柜平面与功率母线", h("div", { className: "floor-plan" }, [
    sourceGroup("JG1", state.racks, state.alarms, state.devices, tooltip),
    sourceGroup("JG2", state.racks, state.alarms, state.devices, tooltip),
    sourceGroup("KT1", state.racks, state.alarms, state.devices, tooltip),
  ...
  ]), { kicker: "单层平面接线图", className: "floor-panel" });
  ...
  const alarmBody = activeAlarms.length
    ...
    }, [statusBadge(severityLabel(alarm.severity), alarm.severity === "critical" ? "danger" : "warning"), h("strong", { text: alarmTriggerLabel(alarm.trigger_code) }), h("span", { text: alarm.object_id })])))
  ...
  container.replaceChildren(hero, metrics, h("section", { className: "overview-grid" }, [floorPlan, alarmPanel]), networkPanel, tooltip);
}
