...
import { createRackCapacityPanel, createRackVisualPanel } from "./rack-detail.js";
...
function rackTopology(result) {
  const rack = result.rack;
  const subject = h("section", { className: "topology-subject" }, [
    h("div", {}, [
      h("p", { className: "section-kicker", text: rack.id }),
      h("h2", { text: `${rack.id} · ${rack.name}` }),
      h("p", { text: `${rack.source_id} / ${rack.height_u}U · 分层线 ${rack.dividers_u.join(" / ")}U` }),
    ]),
    h("div", { className: "intro-badges" }, [
      sourceBadge(rack.data_source),
      statusBadge(rack.role === "network" ? "网络机柜" : "服务器机柜", "neutral"),
    ]),
  ]);
  const onDeviceClick = (device) => openDeviceDetailDialog(device, rack);
  return h("div", { className: "topology-results" }, [
    subject,
    createRackCapacityPanel(rack),
    createRackVisualPanel(rack, { onDeviceClick }),
    h("button", {
      type: "button",
...
function matchAggregate(result) {
  ...
  if (query.startsWith("CAB-")) {
    racks = power.cabinets.filter((rack) => rack.id.includes(query));
    sources = power.power_sources.filter((source) => racks.some((rack) => rack.source_id === source.id));
    switches = network.access_switches.filter((item) => item.connected_rack_ids.some((rackId) => rackId.includes(query)));
  } else if (query.startsWith("JG") || query === "KT1") {
    sources = power.power_sources.filter((source) => source.id.includes(query));
    racks = power.cabinets.filter((rack) => rack.source_id.includes(query));
    ...
    switches = network.access_switches.filter((item) => item.id.includes(query));
  ...
  const result = state.topologyResult
    ? state.topologyResult.kind === "device" ? deviceTopology(state.topologyResult)
      : state.topologyResult.kind === "multi-device" ? multiDeviceTopology(state.topologyResult, state.racks)
        : state.topologyResult.kind === "rack" ? rackTopology(state.topologyResult)
