...
function pathPanel(title, nodes, tone) {
  return panel(title, nodes?.length
    ...
    : emptyState("链路信息未接入", "系统保留未知节点，不会推测或补造实际连接。"), { kicker: tone === "power" ? "POWER PATH" : "NETWORK PATH" });
}
...
function aggregateTopology(result) {
  ...
  const selection = h("section", { className: "topology-selection" }, [
    h("div", {}, [
      h("p", { className: "section-kicker", text: "SELECTED TRACE" }),
  ...
  return h("div", { className: "topology-results" }, [
    selection,
    h("section", { className: "topology-columns" }, [panel("强电对象", powerBody, { kicker: "POWER OBJECTS" }), panel("弱电对象", networkBody, { kicker: "NETWORK OBJECTS" })]),
  ]);
}
...
export function renderTopology(container, state) {
  ...
  const intro = h("section", { className: "view-intro" }, [
    h("div", {}, [h("p", { className: "section-kicker", text: "KNOWN LINKS ONLY" }), h("h2", { text: "强弱电链路证据" }), h("p", { text: "分别展示配电与网络路径。未知连接保持未知，所有节点显示数据来源。" })]),
    statusBadge(collectorOffline ? "采集离线 · 使用设计数据" : "设计链路可查", collectorOffline ? "warning" : "ok"),
  ]);
  const searchPanel = panel("查询对象", [form, quickQueries], { kicker: "TRACE" });
