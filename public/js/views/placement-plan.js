...
function deviceLabel(device) {
  const id = device?.id ?? "设备";
  const itCode = device?.it_code;
  return itCode ? `${id} / IT-Code: ${itCode}` : id;
}

function connectionText(action) {
  const power = (action.power_connections ?? [])
    .map((item) => `${item.pdu_id}/${item.outlet}`)
    .join("、");
  const network = (action.network_connections ?? [])
    .map((item) => `${item.switch_id}/${item.switch_port}`)
    .join("、");
  return [power ? `PDU ${power}` : null, network ? `交换机 ${network}` : null]
    .filter(Boolean)
    .join(" · ");
}

function actionColorBlock(color) {
  if (!color) return null;
  return h("span", { className: "action-color-block", style: { backgroundColor: color } });
}

function planColorFromActions(actions) {
  const placement = actions?.find((action) => action.type === "place_device");
  return placement?.device?.color ?? null;
}

function actionSortKey(type) {
  if (type === "move_device") return 0;
  if (type === "place_device") return 1;
  return 2;
}

function orderedActions(actions) {
  return [...(actions ?? [])].sort((left, right) => actionSortKey(left.type) - actionSortKey(right.type));
}

function actionItem(action, planColor = null) {
  const color = action.type === "place_device" ? action.device?.color : planColor;
  return h("li", { className: "action-item" }, [
    actionColorBlock(color),
    h("span", { text: actionText(action) }),
  ]);
}

function renderActionList(actions) {
  const color = planColorFromActions(actions);
  return h("ol", { className: "action-list" }, orderedActions(actions).map((action) => actionItem(action, color)));
}

function actionText(action) {
  if (action.type === "place_device") {
    return `${actionLabels[action.type]} ${deviceLabel(action.device)} → ${action.rack_id} / ${action.layer_id} / ${intervalText(action.start_u, action.device?.u_size)}`;
  }
  if (action.type === "move_device") {
    const from = action.from ?? {};
    const to = action.to ?? {};
    const links = connectionText(action);
    return `${actionLabels[action.type]} ${action.device_id}：${from.rack_id} / ${from.layer_id} / U${from.start_u} → ${to.rack_id} / ${to.layer_id} / U${to.start_u}${links ? ` · ${links}` : ""}`;
  }
  if (action.type === "remove_device") {
    return `${actionLabels[action.type]} ${action.device_id} ← ${action.rack_id} · 同层下移整理 ${(action.compaction_moves ?? []).length} 台`;
  }
...
function candidateCard(candidate, selected, ruleRank, onSelect) {
  ...
  const placementCount = candidate.actions?.filter((action) => action.type === "place_device").length ?? 0;
  const migrationCount = candidate.actions?.filter((action) => action.type === "move_device").length ?? 0;
  ...
  return h("article", {
    ...
    h("div", { className: "candidate-head" }, [
      ...
      statusBadge(
        candidate.risk === "high_impact"
          ? "扩大迁移范围 / 高影响"
          : candidate.risk === "safe" ? "可执行" : "有风险",
        candidate.risk === "safe" ? "ok" : "warning",
      ),
    ]),
    ...
    h("p", { className: "candidate-impact-summary", text: `新增 ${placementCount} 台 · 迁移 ${migrationCount} 台` }),
...
function capacityComparison(plan) {
  const rackIds = [...new Set((plan.actions ?? []).flatMap((action) => action.type === "move_device"
    ? [action.from?.rack_id, action.to?.rack_id]
    : [action.rack_id]).filter(Boolean))];
...
function compactionImpact(impact = []) {
  return h("section", { className: "compaction-impact" }, [
    h("h4", { text: `同层下移整理：${impact.length} 台` }),
    impact.length > 0
      ? h("ol", { className: "action-list" }, impact.map((move) => h("li", {
        text: `${move.device_id}：U${move.from_start_u} → U${move.to_start_u}（机柜、挡板层和连接保持不变）`,
      })))
      : h("p", { text: "取出后该挡板层无需移动其他服务器。" }),
  ]);
}
...
export function openPlanConfirmation(plan, onComplete) {
  ...
  const body = h("div", { className: "confirmation-content" }, [
    ...
    isRemoval ? compactionImpact(plan.compaction_impact ?? plan.actions?.find((action) => action.type === "remove_device")?.compaction_moves ?? []) : null,
    ...
    h("div", { className: "confirmation-actions" }, [
      h("button", {
        type: "button", className: "primary-button", text: "确认并模拟执行", onClick: async (event) => {
...
export function renderPlacementPlan(plan, state) {
  ...
  return h("div", { className: "plan-result" }, [
    ...
    isRemoval ? h("section", { className: "removal-summary" }, [
      ...
      h("p", { text: `同层下移整理：${(removalAction?.compaction_moves ?? []).length} 台` }),
    ...
    issueList("风险提醒", plan.validation?.warnings, "warning"),
    migrationImpact(plan.migration_impact),
    isRemoval ? compactionImpact(plan.compaction_impact ?? removalAction?.compaction_moves ?? []) : null,
