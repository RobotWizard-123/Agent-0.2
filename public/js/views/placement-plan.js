...
function candidateCard(candidate, selected, ruleRank, onSelect) {
  ...
  const card = h("article", {
    ...
    h("div", { className: "candidate-ranks" }, [
      ...
      h("span", { text: selected ? "Agent 最终选择" : (onSelect ? "点击设为最终候选" : "Agent 未选择") }),
    ]),
  ...
  if (onSelect && !selected) {
    const handler = async (event) => {
      ...
      try {
        ...
        showToast(`${error.code ?? "PLAN_CANDIDATE_FAILED"} · ${error.message ?? "切换候选失败"}`, "danger");
      }
    };
...
export function openPlanConfirmation(plan, onComplete) {
  ...
  const body = h("div", { className: "confirmation-content" }, [
    h("p", { className: "section-kicker", text: "ONE FINAL CONFIRMATION" }),
...
export function renderPlacementPlan(plan, state) {
  ...
  const canSelect = state.session.role === "admin"
    && !["locked", "executing", "succeeded", "failed"].includes(plan.status)
    && (plan.original_candidates ?? []).length > 1;
  const onSelect = canSelect
    ? (candidateId) => selectPlanCandidate(plan.id, candidateId).then(() => showToast("已切换为该候选", "ok"))
    : null;
  return h("div", { className: "plan-result" }, [
    h("div", { className: "plan-status-row" }, [
      h("div", {}, [
        h("p", { className: "section-kicker", text: `${plan.id} · SNAPSHOT ${plan.snapshot_version ?? "—"}` }),
    ...
    isRemoval ? h("section", { className: "removal-summary" }, [
      h("p", { className: "section-kicker", text: "REMOVE / VERIFY / AUDIT" }),
    ...
    ]) : h("div", { className: "candidate-grid" }, (plan.original_candidates ?? []).slice(0, 4).map((candidate, index) => candidateCard(candidate, candidate.id === selected, index + 1, onSelect))),
