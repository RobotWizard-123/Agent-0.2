...
- 上架 Agent 使用统一重配置模型比较直接部署与服务器迁移；同规格批次一次确认、原子执行，跨机柜迁移同步分配 PDU 与交换机端口，常规范围无解时自动扩大迁移范围。
...
构建和启动包含 CP-SAT 运行时的容器：

```powershell
docker build -t datacenter-mgr:cp-sat .
docker run --rm -p 127.0.0.1:3000:3000 --env-file .env --env HOST=0.0.0.0 --env CP_SAT_PYTHON=/opt/venv/bin/python datacenter-mgr:cp-sat
```

最后的 `--env HOST` 和 `--env CP_SAT_PYTHON` 明确覆盖了本地 `.env` 中的 Windows 路径与进程监听设置；容器内进程监听 `0.0.0.0`，但发布端口只绑定宿主机回环地址。远程访问必须使用下文记录的 HTTPS Compose profile。禁用 CP-SAT 或 Python 运行时不可用时，规划仍会使用 Beam Search 回退，不会停止规划功能。
...
默认只绑定 `127.0.0.1:3000`。远程访问必须使用 HTTPS Compose profile，不直接发布应用容器端口。
